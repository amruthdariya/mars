...
const { dealerpartner } = require('../pages/dealerpartner');
...
test('1-D2C Ticket Creation', async ({ page }) => {
  ...
  const loginPage1 = new LoginPage(page,testData);
  tagId = generateTagId();
  console.log('tagid generated.................',tagId);
 // const url="https://rims-demo.blubirch.com/";
  await loginPage1.navigate();
  await loginPage1.login();
  await loginPage1.verifyLogin();
  
// Return Creation
  const dealerpage = new dealerpartner(page,testData);
  await dealerpage.verifyModuleAndStartRequest();
  await dealerpage.fillCustomerDetails();
  await dealerpage.fillItemDetails(undefined, undefined, tagId);
  await dealerpage.submitAndVerifyOTP();
   global.subrequestId = await dealerpage.captureSubRequestId();
  await dealerpage.searchsubrequestId(global.subrequestId );
  //await dealerpage.assignServicePartner();
  //await page.waitForTimeout(4000);
  //await dealerpage.assignServiceTechnician();
});
//SERVICE INSPECTION
...
test('4-Logistics Details Update By Logistics Manager', async ({ page }) => {
  test.setTimeout(60000);
  const rlPage1=new reverselogisticspage(page,testData5);
  rlPage1.navigate();
  rlPage1.login();
  await rlPage1.navigateToReverseLogistics();
  await rlPage1.searchTicket(subrequestId);   // replace with the subrequestId
  await rlPage1.updateConfirmation();
  await rlPage1.updateAssignment();
  await rlPage1.updatePickup();
  await rlPage1.updateDrop();
  // Logout
  //await rlPage.logout();
